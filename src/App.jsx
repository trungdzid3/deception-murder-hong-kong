...
import { MASTER_DIRECTORY } from './data/master-directory';
...
function App() {
  ...
  const handleVisitNode = (nodeId) => {
    ...
    // Cập nhật ngay lập tức vào roomState cá nhân (UI Đã khám xét tăng lập tức 0ms delay)
    setRoomState(prev => {
      if (!prev) return { visitedNodes: [nodeId] };
      const currentVisited = prev.visitedNodes || [];
      if (currentVisited.includes(nodeId)) return prev;
      return {
        ...prev,
        visitedNodes: [...currentVisited, nodeId]
      };
    });
                ...
                <div className="space-y-3">
                  ...
                  <div className="sherlock-case-selection-grid">
                    {SHERLOCK_CASES_LIST.map((c, index) => {
                      ...
                      const diffClass = c.diffLevel === 'beginner' ? 'beginner' : c.diffLevel === 'easy' ? 'easy' : c.diffLevel === 'medium' ? 'medium' : c.diffLevel === 'hard' ? 'hard' : c.diffLevel === 'expert' ? 'expert' : 'easy';
                  ...
                  <div className="sherlock-map-wrapper">
                    ...
                    <div className="sherlock-map-viewport">
                      ...
                      {Object.values(activeSherlockCase?.nodes || {}).map((n) => {
                        ...
                        const pos = (n.map_coords?.left && n.map_coords?.top) ? n.map_coords : (coordsMap[n.id] || { left: '50%', top: '50%' });
                    ...
                    <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 border-b border-amber-500/20 pb-3">
                      ...
                      <div className="relative w-full md:w-64">
                        ...
                        <input
                          ...
                          className="sherlock-search-input pl-8"
                        />
                      </div>
                    </div>
                      ...
                      {activeSherlockCase.newspaper?.articles?.map((art, i) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {art.full_text && (
                              <div className="sherlock-newspaper-read-link">
